// WordPress软文发布中间件 V2.2 - 前端脚本
// 支持富文本编辑器、配置管理和多角色登录系统
...
async function handleFormSubmit(event) {
    ...
    const formData = {
        ...
        // V2.2版本：不再需要author_token，通过登录状态验证
    };
...
function updateServiceStatus(online, data = null) {
    ...
    if (online) {
        ...
        statusText.textContent = 'V2.2服务正常运行';
        ...
        // 显示活跃会话数
        if (data && data.active_sessions !== undefined) {
            statusText.textContent += ` | 活跃会话: ${data.active_sessions}`;
        }
    } else {
...
function saveFormData() {
    ...
    localStorage.setItem('formData_v2_2', JSON.stringify(formData));
}

// 从localStorage恢复表单数据
function restoreFormData() {
    const savedData = localStorage.getItem('formData_v2_2');
    if (savedData) {
        try {
...
function exportHistory() {
    ...
    link.setAttribute('download', `发布历史_V2.2_${new Date().toISOString().split('T')[0]}.csv`);
